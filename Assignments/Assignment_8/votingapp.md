...
- redis
    ...
    ```
    docker service create --name redis --network frontend --replica 2 redis:3.2
    ```
- worker
...
- db
    ...
    - on backend network
    - 1 replica
    - creating volume require sometime different from using volume `-v`. Use the new `--mount` format to do this.
```
